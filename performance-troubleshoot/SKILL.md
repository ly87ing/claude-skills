...
graph TD
    ...
    C -->|突然暴涨| C1[对象创建风暴]
    C -->|持续上涨| C2[资源泄露]
    C1 --> C1R["✅ 推荐: 对象池/结果复用/版本追踪/批量限流"]
    C2 --> C2R["✅ 推荐: 生命周期管理/定期清理/弱引用"]
    
    B -->|GC压力| D{类型?}
    D -->|Young GC频繁| D1["短命对象过多"]
    D -->|Full GC频繁| D2["老年代满"]
    D1 --> D1R["✅ 推荐: 对象池/预分配/减少临时对象"]
    D2 --> D2R["✅ 推荐: 缓存淘汰/资源清理/堆大小调整"]
    
    B -->|响应慢| E{原因?}
    E -->|锁竞争| E1["锁粒度大"]
    E -->|IO阻塞| E2["下游慢"]
    E1 --> E1R["✅ 推荐: 锁分段/读写锁/无锁设计"]
    E2 --> E2R["✅ 推荐: 异步化/缓存/熔断器"]
    
    B -->|消息积压| F{原因?}
    F -->|消费慢| F1["处理能力不足"]
    F -->|生产快| F2["突发流量"]
    F1 --> F1R["✅ 推荐: 批量消费/并行消费/消费者扩容"]
    F2 --> F2R["✅ 推荐: 背压/限流/队列溢出策略"]
```
